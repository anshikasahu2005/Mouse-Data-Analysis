...
if it has a processed/dFF/ folder. A "Comparative Analysis" option in the
subject dropdown overlays all discovered mice against each other.
...
COMPARE_LABEL = "📊 Comparative Analysis (All Mice)"
...
@st.cache_data
def load_all_summaries(subjects_tuple):
    frames = []
    for s in subjects_tuple:
        _, _, _, _, _, summary = load_subject(s)
...
# ============================================================================
#  COMPARATIVE VIEW (ALL MICE)
# ============================================================================
def render_comparative(subjects):
    st.sidebar.caption(f"Comparing {len(subjects)} mice: {', '.join(subjects)}")
    st.sidebar.markdown("---")
    st.sidebar.caption(
        "Every mouse is scored with the same lick-cross-referencing method (response "
        "window = onset + 1.0-2.5s), so accuracy/d-prime/etc. are directly comparable "
    ...
    imaging_flags = {s: has_imaging(s) for s in subjects}

    st.header("Comparative Analysis — All Mice")
    ...
    for i, s in enumerate(subjects):
        ...
        with cols[i]:
            st.markdown(f"**{s}**")
            st.metric("Sessions", sub.shape[0])
            st.metric("Trials", f"{int(sub.n_trials.sum()):,}")
            st.metric("Mean accuracy", f"{sub.accuracy.mean()*100:.0f}%" if sub.accuracy.notna().any() else "—")
            st.metric("Best d′", f"{sub.dprime.max():.2f}" if sub.dprime.notna().any() else "—")
            st.caption("🧠 has imaging" if imaging_flags[s] else "no imaging")

    tabs = st.tabs(["📈 Learning Curves", "📊 Session & Trial Stats", "🗂️ Data Availability"])

    with tabs[0]:
        st.subheader("Overlaid learning curves")
        metric = st.radio("Metric", ["accuracy", "hit_rate", "fa_rate", "dprime", "criterion"],
                           horizontal=True, key="cmp_metric")
        fig = px.line(all_summary, x="day_number", y=metric, color="subject", markers=True,
                      color_discrete_map=subject_colors, hover_data=["session"],
                      labels={"day_number": "Training day #"})
        if metric in ("accuracy", "hit_rate", "fa_rate"):
            fig.add_hline(y=0.5, line_dash="dot", line_color="grey")
        else:
            fig.add_hline(y=0, line_dash="dot", line_color="grey")
        st.plotly_chart(fig, use_container_width=True, key="cmp_learning")
        st.caption("x-axis is each mouse's own training-day number (day 1 = their first recorded session), "
                   "not calendar date, so the mice line up by training progress rather than by date.")

        st.subheader("Small multiples — all four metrics at once")
        long = all_summary.melt(id_vars=["subject", "day_number", "session"],
                                 value_vars=["hit_rate", "fa_rate", "accuracy", "dprime"],
                                 var_name="metric", value_name="value")
        fig2 = px.line(long, x="day_number", y="value", color="subject", facet_row="metric",
                       color_discrete_map=subject_colors, height=700,
                       labels={"day_number": "Training day #"})
        fig2.update_yaxes(matches=None)
        st.plotly_chart(fig2, use_container_width=True, key="cmp_small_multiples")

    with tabs[1]:
        st.subheader("Total trials per mouse")
        totals = all_summary.groupby("subject").n_trials.sum().reset_index()
        fig = px.bar(totals, x="subject", y="n_trials", color="subject", color_discrete_map=subject_colors)
        st.plotly_chart(fig, use_container_width=True, key="cmp_trials")

        c1, c2 = st.columns(2)
        with c1:
            st.subheader("Accuracy distribution per mouse")
            fig = px.box(all_summary, x="subject", y="accuracy", color="subject", color_discrete_map=subject_colors)
            st.plotly_chart(fig, use_container_width=True, key="cmp_acc_box")
        with c2:
            st.subheader("Session duration distribution")
            fig = px.box(all_summary, x="subject", y="session_duration_min", color="subject",
                        color_discrete_map=subject_colors, labels={"session_duration_min": "Duration (min)"})
            st.plotly_chart(fig, use_container_width=True, key="cmp_duration_box")

        st.subheader("Mean lick rate per mouse")
        fig = px.bar(all_summary.groupby("subject").lick_rate_hz.mean().reset_index(),
                     x="subject", y="lick_rate_hz", color="subject", color_discrete_map=subject_colors,
                     labels={"lick_rate_hz": "Mean lick rate (Hz)"})
        st.plotly_chart(fig, use_container_width=True, key="cmp_lick_rate")

    with tabs[2]:
        st.subheader("What data each mouse has")
        rows = []
        for s in subjects:
            sub = all_summary[all_summary.subject == s]
            n_imaging_days = 0
            phases = ""
            if imaging_flags[s]:
                dff = load_dff_traces(s)
                n_imaging_days = dff.session.nunique()
                ph = sorted(p for p in dff.phase.unique() if p)
                phases = ", ".join(ph) if ph else "—"
            rows.append({
                "subject": s,
                "sessions": sub.shape[0],
                "first_session": sub.session.iloc[0] if sub.shape[0] else "—",
                "last_session": sub.session.iloc[-1] if sub.shape[0] else "—",
                "total_trials": int(sub.n_trials.sum()),
                "sessions_with_zero_licks": int((sub.n_licks == 0).sum()),
                "has_imaging": "Yes" if imaging_flags[s] else "No",
                "imaging_days": n_imaging_days,
                "imaging_phases": phases,
            })
        st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
        st.caption("\"Sessions with zero licks\" usually means no lick_history file existed for that "
                   "session in the raw data, rather than the mouse truly never licking.")

        st.subheader("Combined session table (all mice)")
        st.dataframe(all_summary, use_container_width=True, hide_index=True, height=400)
        st.download_button("⬇ Download combined session summary", all_summary.to_csv(index=False),
                            "all_mice_session_summary.csv", key="cmp_dl")
...
subject_choice = st.sidebar.selectbox("Subject", [COMPARE_LABEL] + subjects)
